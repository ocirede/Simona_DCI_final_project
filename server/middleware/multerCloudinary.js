...
//Post image upload
const portfoolioImageStorage = new CloudinaryStorage({
  cloudinary: cloudinaryV2,
  params: {
    folder: "Simona_Final_Project/portfolio_images",
    format: async (req, file) => {
      try {
        if (!file.mimetype.startsWith("image")) {
          throw new Error("Only image files are allowed");
...
const portfolioImageUpload = multer({ storage: portfoolioImageStorage });
