...
const OfferProvider = ({ children }) => {
    ...
    const fetchOffers = async () => {
        try {
            ...
            if(response.data.success) {setOffers(response.data.offers)
            }
        } catch (error) {
...
const deleteOffer = async (offerId) => {
        try {
           const response = await axios.delete(baseURL + `/offers/${offerId}`);
            if(response.data.success){
                ...
                setOffers(prevOffers => prevOffers.filter(offer => offer._id !== deletedOffer))
    ...
    const updateOffer = async (offerId, updateData) => {
        try {
            const response = await axios.put(baseURL + `/offers/${offerId}`, updateData);
            if(response.data.success){
    ...
    const applyOffer = async (offerId, applicantId) => {
        try {
          const response = await axios.post(baseURL + '/offers/apply', { offerId, applicantId });
          if(response.data.success) {
    ...
    return (
        <OfferContext.Provider value={{ offers, createOffer, updateOffer, deleteOffer,applyOffer }}>
            {children}
